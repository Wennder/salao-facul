...
class VendaMySqlDAO implements VendaDAO{
	...
	public function insert($venda){
		$sql = 'INSERT INTO venda (cliente_id, data, data_vencimento, total, total_pago, obs) VALUES (?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
		
		$sqlQuery->setNumber($venda->clienteId);
		$sqlQuery->set($venda->data);
		$sqlQuery->set($venda->dataVencimento);
	...
	public function update($venda){
		$sql = 'UPDATE venda SET cliente_id = ?, data = ?, data_vencimento = ?, total = ?, total_pago = ?, obs = ? WHERE id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	public function queryByDataVencimento($value){
		$sql = 'SELECT * FROM venda WHERE data_vencimento LIKE ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	public function deleteByDataVencimento($value){
		$sql = 'DELETE FROM venda WHERE data_vencimento = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	protected function readRow($row){
		...
		$venda->data = $row['data'];
		$venda->dataVencimento = $row['data_vencimento'];
